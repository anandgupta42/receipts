...
- [ ] `docs/pr-receipts.md` has a "one command" contributor path that starts with
      `npx aireceipts pr --post` and demotes aliases/hooks to optional convenience.
- [ ] The PR template Evidence block asks for the receipt generated by that exact
      command.
- [ ] A single assistant-instruction snippet is documented for all coding assistants.
...
- [ ] SPEC-0036 workflow/docs still say CI checks presence only and is notice-only by
      default.
- [ ] Acceptance testing performed live with command + output recorded in this spec's
