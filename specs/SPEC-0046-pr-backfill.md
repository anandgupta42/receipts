...
**Parked 2026-07-05 — worth gate (S3) not met.** Fully buildable and correct
(Codex S2 REWORK → 6/6 applied), but an edge case: a recovery tool for
backfilling a receipt onto an already-merged PR — i.e. "I forgot to run
`pr --post` before merging." The motivating need (cleaning up this session's own
over-attributed receipts) was already handled honestly without it: the 5 wrong
receipts were deleted, #128 got a clean receipt, and the remaining merged PRs are
honestly receipt-less (an acceptable do-nothing outcome). The general case is
rare, and the repo already has a check that *notices* a skipped receipt before
merge. Revive only if "backfill after merge" becomes a recurring, user-reported
need. This spec is the motivating incident for the S3 worth-building gate added
to `/validate-spec` — a spec can survive the correctness attack (S2) and still
